...
from PlanificadorViajes.AgentUtil.ACLMessages import get_message_properties, build_message, register_agent
...
def comunicacion():
    ...
    if msgdic is None:
        ...
        if performative != ACL.request:
            ...
            if accion == Ontologia.Peticion_Valorados:
                get_all_sells()
                ...
                return serialize, 200
            elif accion == Ontologia.Peticion_valorar:
                for item in gm.subjects(RDF.type, ACL.FipaAclMessage):
                    gm.remove((item, None, None))
                for item in gm.subjects(RDF.type, Ontologia.Peticion_valorar):
                    gm.remove((item, None, None))
